...
async function sendThankYouEmail(email: string, city: string) {
  ...
  const htmlContent = `
    <div style="font-family: 'Georgia', serif; max-width: 600px; margin: 0 auto; background-color: #f9f7f4; padding: 30px; border-radius: 10px;">
      ...
      <h2 style="color: #8B4513; text-align: center;">Благодарим ти, че се присъедини към движението!</h2>
      ...
      <p style="color: #5D4037; font-size: 16px; line-height: 1.6;">
        Благодарим ти, че се присъедини към нашето движение за връщане на истинския български вкус! 
        Твоето участие в quiz-а ни помага да разберем по-добре какво търсят хората като теб.
      </p>
  ...
  console.log('[VERCEL] Sending email with Resend to:', email);

  return resend.emails.send({
    from: 'Бачо Илия <onboarding@resend.dev>', // Resend използва само verified домейни
    to: email,
